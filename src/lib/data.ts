...
import { collection, getDocs, getDoc,startAfter,DocumentSnapshot, doc, query, where, orderBy, limit, addDoc, serverTimestamp, updateDoc, deleteDoc, setDoc, Query, and, QueryConstraint, QueryFilterConstraint, documentId, increment } from 'firebase/firestore';
...
): Promise<{ data: Job[]; lastVisible: DocumentSnapshot | undefined }> {
  
  try {
    const { pageLimit = 10, lastVisible, searchQuery, excludeId} = options;
    ...
    return { 
        data: allJobs, 
        lastVisible: lastVisibleDoc || undefined 
    };
